...
# Optional providers; any missing ones are just skipped by the fallback chain
...
def _mkdirs():
    for p in (OUT_BUNDLE, OUT_METRICS, DATA_MIRROR, ROOT / "outputs" / "proe"):
...
def _write_csv(df: pd.DataFrame, path: Path) -> None:
    """
    Always write a CSV with headers. If df is empty or invalid, we still write
    an empty CSV with headers when possible to keep the validator happy.
    """
    ...
    if _ok(df):
        ...
        # Try to preserve schema if we were given an empty-but-typed DataFrame
        if isinstance(df, pd.DataFrame) and len(df.columns) > 0:
            df.head(0).to_csv(path, index=False)
        else:
            pd.DataFrame().to_csv(path, index=False)
        print(f"[write-empty] {path.relative_to(REPO)}  rows=0")
...
# ---------------- Primary paths (already-fetched artifacts) ----------------
...
def _get_or_resolve(name: str, season: int, cache: Dict[str, pd.DataFrame]) -> pd.DataFrame:
    """
    Try previously-cached, then on-disk 'primary' CSVs, then providers in FALLBACKS.
    """
    df = cache.get(name)
    if _ok(df):
        return df
    # primary artifacts written by fetchers/addons
...
# ---------------- Derived metrics (unchanged from your logic) ----------------
...
def derive_team_from_pbp(pbp: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    if epa:
        try:
            off_pass = (pbp.loc[pass_flag & pbp[posteam].notna(), [posteam, epa]]
                        .groupby(posteam, as_index=False)[epa].mean()
                        .rename(columns={posteam: "team", epa: "off_pass_epa"}))
            off_rush = (pbp.loc[rush_flag & pbp[posteam].notna(), [posteam, epa]]
                        .groupby(posteam, as_index=False)[epa].mean()
    ...
    if air:
        attempts = (pbp.loc[pbp[posteam].notna(), [posteam]]
                    .assign(att=pass_flag.astype(int))
                    .groupby(posteam, as_index=False)["att"].sum()
                    .rename(columns={posteam: "team"}))
        air_sum = (pbp.loc[pass_flag & pbp[posteam].notna(), [posteam, air]]
    ...
    if sec_rem and score_diff:
        ...
        p["sec"] = pd.to_numeric(p[sec_rem], errors="coerce")
    ...
    down = _pick(pbp, "down")
    ytg  = _pick(pbp, "ydstogo","yards_to_go","yds_to_go")
    yline_use = yline
    weekcol = weekcol
    ...
    env["rz_rate"] = 0.0
    if yline:
        df = (pbp.loc[pbp[posteam].notna() & pbp[yline].notna(), [game_id, posteam, yline]]
              .copy()
              .rename(columns={posteam: "team"}))
...
def derive_player_from_pbp(pbp: pd.DataFrame) -> pd.DataFrame:
    ...
    posteam  = _pick(pbp, "posteam","offense_team","PossessionTeam") or "posteam"
    rec_name = _pick(pbp, "receiver_player_name","receiver","receiver_name",
    ...
    if yline:
        rz_mask = pd.to_numeric(pbp[yline], errors="coerce") <= 20
    ...
    for c in ["target_share","rush_share","rz_tgt_share","rz_carry_share","yprr_proxy","ypc","ypt","qb_ypa"]:
        out[c] = pf[c] if c in pf.columns else 0.0
...
def compose_team_form(season: int, cache: Dict[str, pd.DataFrame]) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    # Optional: integrate box rates if present
...
def compose_player_form(season: int, cache: Dict[str, pd.DataFrame]) -> pd.DataFrame:
    ...
    cols = ["player","team","position","target_share","rush_share","rz_tgt_share","rz_carry_share","yprr_proxy","ypc","ypt","qb_ypa"]
    for c in cols:
        if c not in pf.columns:
            pf[c] = "" if c in ("player","team","position") else 0.0
    return pf[cols]

# ---------------- Fetch bundle & addons (NEW) ----------------
def fetch_bundle(seasons: List[int]) -> None:
    """
    1) If present, run external/nflverse_bundle/fetch_all.py for each season.
    2) Then run addon scripts for each season. Failures are logged as warnings;
       we keep going so compose can fall back to providers when needed.
    """
...
def main() -> int:
    ...
    # NEW: run the actual fetchers/addons before composing metrics
