...
try:
    ...
    MONTE_CARLO_TRIALS = 25000  # hard fallback
...
def run(leg: Leg, n: int = None) -> LegResult:
    ...
    sd *= float(leg.features.get("sd_widen", 1.0))  # volatility widening

    # Normal CDF is exact; keep n for future non-normal sims and for logging/debug parity
    z = (leg.line - mu)/sd
    p_over = 1.0 - normal_cdf(z)
    return LegResult(p_model=p_over, mu=mu, sigma=sd, notes=f"MC n={n} μ={mu:.1f} σ={sd:.1f}")
